...
The main difference between these two is their intended usage.  In its core,
...
- Persistent test results - logs, results, "generated garbage" is stored between
  runs in the Common Test, which allows tracking what and when happened, and if
...
it's API isn't the top of the class. It happens a lot in Erlang tools, whoever
...
Another part, and the one that I miss in ExUnit, is reporting in Common Test.
This alone is in my humble opinion **THE** best part of the library. To present
you how this work assume that we have above test suite which we will run with
`ct_run -dir .`. This will compile and run our tests (obviously), but in
...
Just check [this out](/common-test-example/simple/index.html). This is example
report generated by the Common Test. As you can see it contains a lot of
information in readable format. Not only it contains information about current
run, but all previous runs as well, which is really handy when tracking
regressions.
...
tests, for example, let's modify our test to log some information:
...
due to its popularity and support in many CI services.
...
I am very interested in your feelings about Common Test usage in Elixir and
